...
(function($) {
	...
	$ss.fn.extend({
		...
		fadeInAnimat : function(){
			var self = this;
			$(".box_skitter_data div[rel='" + self.settings.oldSlide + "']", self.slide).animate({opacity:0,zIndex:1, left:($(".box_skitter_data div[rel='" + self.settings.oldSlide + "']", self.slide).width()%5)+'px',width:120 +'%'}, self.settings.animation);
