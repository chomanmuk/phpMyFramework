	...
	$ss.fn.extend({
		...
		makeLayer : function(){
			...
			for(var i=0;i<self.settings.widthcut;i++){
				$(".box_skitter_data", self.slide).append("<div style='position:absolute;width:" + layerwidth + "px;left:" + (i*layerwidth) + "px;z-index:10;height:" + $(".box_skitter_data div[rel='" + self.settings.oldSlide + "'] > img", self.slide).height() + "px;background-image:url(" + $(".box_skitter_data div[rel='" + self.settings.oldSlide + "'] > img", self.slide).attr('src') + ")'></div>")
			}
		},
