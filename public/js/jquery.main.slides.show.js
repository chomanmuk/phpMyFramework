...
(function($) {
	...
	$ss.fn.extend({
		...
		fadeInAnimat : function(){
			...
			$(".box_skitter_data div[rel='" + self.settings.oldSlide + "']", self.slide).attr('on', 'N').css({opacity:1,left:0+'px',width:100+'%'}).stop().animate({opacity:0,zIndex:1, left:'-'+($(".box_skitter_data div[rel='" + self.settings.oldSlide + "']", self.slide).width()*0.5)/2+'px',width:100 +'%'}, self.settings.animation);
